...
Every row above is a first visit with an empty cache. Loading the same page
repeatedly in one browser, good connection, no throttling:
...
So a returning visitor sees roughly 1.7 s and a first-time visitor sees
roughly 4 s on the same connection. The caching is working correctly. The
finding is about the first visit, because that is the one a new customer
arriving from an advert gets.
```

**Why it matters.** Every figure here is a first visit with an empty cache, and that is the only case that matters for this finding: someone clicking through from an advert or a search result has never been to your site before. Visit the same page a second time and it comes back in about one and a half seconds, because by then almost everything is being read from the phone's own storage rather than fetched. That is the gap worth understanding, because it is why the shop can feel quick to you, to your team and to anyone who has been on it before, and still be slow for the new customer you paid to bring there. For that first-time visitor on a good connection these pages take about four seconds, against about one and a half for your home page and your product pages, so the category template is roughly two and a half times slower than the rest of the site. Four seconds is already past the two and a half that Google counts as good, while the home page and product pages clear that bar comfortably, so the problem is specific to the category template rather than to the site or the server. On a weak mobile signal the same first visit stretches to about fourteen seconds, which is what the industry-standard mobile test reports and what Google's PageSpeed score is based on. The two findings below are the cause of all of these figures.
